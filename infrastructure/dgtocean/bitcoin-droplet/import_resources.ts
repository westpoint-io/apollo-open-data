...
const resources = [
  {
    env: "EXISTING_VOLUME_ID",
    type: "digitalocean:index/volume:Volume",
    staticName: "existingVolume",
  },
  {
    env: "EXISTING_DROPLET_ID",
    type: "digitalocean:index/droplet:Droplet",
    staticName: "existingDroplet",
  },
];
...
async function importResource(resource: {
  ...
  const resourceId = process.env[resource.env];
  if (!resourceId) {
  ...
  try {
    ...
    const constMatch = (stdout + stderr).match(
      /const\s+\w+\s*=\s*new[\s\S]*?\}\);/
    );
    if (constMatch) {
      let constCode = constMatch[0];
      constCode = constCode.replace(
        /const\s+\w+/,
        `const ${resource.staticName}`
      );
      const exportedCode = `export ${constCode}\n\n`;
      fs.appendFileSync("generated_resources.ts", exportedCode);
      console.log(
        `Appended generated code for ${resourceId} as ${resource.staticName} to generated_resources.ts`
      );
    } else {
    ...
    if (error.stdout) {
      const constMatch = error.stdout.match(
        /const\s+\w+\s*=\s*new[\s\S]*?\}\);/
      );
      if (constMatch) {
        let constCode = constMatch[0];
        ...
        fs.appendFileSync("generated_resources.ts", exportedCode);
        console.log(
          `Appended generated code for ${resourceId} as ${resource.staticName} to generated_resources.ts (from error output)`
        );
      }
    }
  }
...
`;
  ...
  if (!existingContent.includes(importStatements.trim())) {
    fs.writeFileSync(
      "generated_resources.ts",
      importStatements + existingContent
    );
